...
const OCCASIONS = [
  { day: 'Mon', label: 'Office/Formal', icon: '💼', weather: 'Sunny' },
  { day: 'Tue', label: 'Casual Tech', icon: '💻', weather: 'Chilly' },
  { day: 'Wed', label: 'Mid-week Brunch', icon: '☕', weather: 'Warm' },
  { day: 'Thu', label: 'Client Meeting', icon: '🤝', weather: 'Cloudy' },
  { day: 'Fri', label: 'Party Night', icon: '🕺', weather: 'Breezy' },
  { day: 'Sat', label: 'Weekend Trip', icon: '🚗', weather: 'Sunny' },
  { day: 'Sun', label: 'Date/Dine', icon: '🍝', weather: 'Pleasant' }
];
...
function OutfitCalendar({ bestColors, pantColors, gender, isDark, onClose }) {
  ...
  const getOutfitForDay = (index) => {
    const top = bestColors[index % bestColors.length] || { name: 'White Tee', hex: '#FFFFFF' };
    const bottom = pantColors[(index + 2) % pantColors.length] || { name: 'Denim', hex: '#1e3a8a' };
    ...
    <div className={`flex flex-col h-full animate-fade-in ${isDark ? 'text-white' : 'text-gray-900'}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          <h2 className="text-xl font-black uppercase tracking-tight">AI Outfit Calendar</h2>
          <p className="text-[10px] opacity-60 uppercase font-black tracking-widest leading-none">Smart Weekly Drops</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-4 mb-6">
        {WEEKDAYS.map((day, i) => (
          ...
          >
            <span className="text-[10px] font-bold uppercase tracking-widest">{day.slice(0, 3)}</span>
      ...
      <div className={`flex-1 rounded-[32px] p-6 border relative overflow-hidden flex flex-col items-center justify-center text-center ${
        ...
        <div className="relative z-10 mb-4">
           ...
           <h3 className="text-2xl font-black mb-6">{WEEKDAYS[selectedDay]}'s Drop</h3>
        </div>
      ...
      <p className="mt-4 text-[10px] text-center opacity-40 font-medium italic">
        "Smart logic ensures colors contrast perfectly with your unique skin tone."
      </p>
    </div>
  );
}
